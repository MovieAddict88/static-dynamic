...
// Display and clear any status messages from redirects
...
// Fetch configured servers to be used in the form and processing
$stmt = $pdo->prepare("SELECT setting_value FROM settings WHERE setting_key = 'auto_embed_servers'");
$stmt->execute();
$configured_servers_json = $stmt->fetchColumn();
$configured_servers = $configured_servers_json ? json_decode($configured_servers_json, true) : [];

// Helper function to parse a URL into a base and a path
function parse_server_url($url, $server_list) {
    foreach ($server_list as $server_base) {
    ...
    try {
        if ($action === 'save') {
            ...
            if (isset($_POST['servers'])) {
                $process_servers($_POST['servers'], $contentId);
            }
                ...
                foreach ($_POST['seasons'] as $season_id => $season_data) {
                     foreach ($season_data['episodes'] as $episode_id => $episode_data) {
                        if (isset($episode_data['servers'])) {
                            $process_servers($episode_data['servers'], null, $episode_id);
                        }
                     }
                }
            }
            ...
            // This action adds missing configured servers without deleting existing ones.
            $content_info = $pdo->query("SELECT type, tmdb_id FROM content WHERE id = $contentId")->fetch();
            $links_added = 0;

            if ($content_info['type'] === 'movie') {
                $server_urls = $pdo->query("SELECT url FROM servers WHERE content_id = $contentId")->fetchAll(PDO::FETCH_COLUMN, 0);
                foreach ($configured_servers as $base_url) {
                    $expected_url = $base_url . $content_info['tmdb_id'];
                    if (!in_array($expected_url, $server_urls)) {
                ...
                foreach($seasons as $season) {
                    ...
                    foreach($episodes as $episode) {
                        $server_urls = $pdo->query("SELECT url FROM servers WHERE episode_id = {$episode['id']}")->fetchAll(PDO::FETCH_COLUMN, 0);
                        foreach ($configured_servers as $base_url) {
                            $expected_url = $base_url . $content_info['tmdb_id'] . '-s' . $season['season_number'] . '-e' . $episode['episode_number'];
                            if (!in_array($expected_url, $server_urls)) {
                                $name = parse_url($base_url, PHP_URL_HOST);
            ...
            $_SESSION['status_message'] = "Operation successful. Added {$links_added} new server link(s).";
...
<div class="container">
    ...
    <div class="card">
        <form action="edit_content.php?id=<?php echo $contentId; ?>&action=save" method="POST">
            ...
            <?php
            function render_server_inputs($servers, $name_prefix, $configured_servers, $contentId) {
                echo "<div class='servers-header'>";
                echo "<h3><i class='fas fa-link'></i> Servers</h3>";
                echo "<form action='edit_content.php?id={$contentId}&action=apply_servers' method='POST' style='display: inline;'>
                        <button type='submit' class='btn btn-secondary btn-small'>Apply Configured Servers</button>
                      </form>";
                echo "</div>";
                ...
                foreach ($servers as $server) {
                    $parsed = parse_server_url($server['url'], $configured_servers);
                    $id = $server['id'];
                    echo "<div class='server-item' id='server-item-{$id}'>";
                    echo "<select name='{$name_prefix}[{$id}][base]'>";
                    foreach ($configured_servers as $base_url) {
                    ...
                    $custom_selected = ($parsed['base'] === 'custom') ? 'selected' : '';
                    echo "<option value='custom' {$custom_selected}>Custom URL</option>";
                    echo "</select>";
                    echo "<input type='text' name='{$name_prefix}[{$id}][path]' value='" . htmlspecialchars($parsed['path']) . "' placeholder='Video ID/Path or Full URL'>";
                    echo "<button type='button' class='btn btn-danger btn-small' onclick='document.getElementById(\"server-item-{$id}\").remove()'>Remove</button>";
                    echo "</div>";
                }
                echo "</div>";
            ...
            <?php if ($content['type'] !== 'series'): ?>
                <?php render_server_inputs($content['servers'], 'servers', $configured_servers, $contentId); ?>
            <?php else: ?>
            <div class="form-group">
                 <div class='servers-header'>
                    <h3><i class="fas fa-list-ul"></i> Seasons & Episodes</h3>
                    <form action='edit_content.php?id=<?php echo $contentId; ?>&action=apply_servers' method='POST' style='display: inline;'>
                        <button type='submit' class='btn btn-secondary btn-small'>Apply Configured Servers to All Episodes</button>
                    </form>
                </div>
                <?php foreach ($content['seasons'] as $season): ?>
                <div class="season-group">
                    <h4>Season <?php echo $season['season_number']; ?></h4>
                    <?php foreach ($season['episodes'] as $episode): ?>
                    <div class="episode-group">
                        <h5>Episode <?php echo $episode['episode_number']; ?>: <?php echo htmlspecialchars($episode['title']); ?></h5>
                        <?php
                        $name_prefix = "seasons[{$season['id']}][episodes][{$episode['id']}][servers]";
                        // For series, the apply button is global, so we don't render it for each episode
                        echo "<div class='servers-container' id='servers-{$name_prefix}'>";
...
<script>
const configuredServers = <?php echo json_encode($configured_servers); ?>;
function addServer(containerId, namePrefix) {
    const container = document.getElementById(containerId);
    const newItem = document.createElement('div');
    newItem.className = 'server-item';
    const newIndex = 'new_' + Date.now();
    let optionsHtml = configuredServers.map(url => `<option value="${escapeHTML(url)}">${escapeHTML(new URL(url).hostname)}</option>`).join('');
    optionsHtml += `<option value="custom">Custom URL</option>`;
    newItem.innerHTML = `<select name="${namePrefix}[${newIndex}][base]">${optionsHtml}</select>
